...
class WaypointUpdater(object):
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if self.pose is not None and self.waypoints_tree is not None:
                closest_waypoint_i = self.get_closest_waypoint_id()
                self.publish_waypoints(closest_waypoint_i)
            rate.sleep()
    ...
    def get_closest_waypoint_id(self):
        ...
        dot = np.dot(pos_coords - closest_coords, prev_coords - closest_coords)
        if dot < 0:
            # closest point is behind
            closest_i += 1
            closest_i %= len(self.waypoints_2d)
        return closest_i

    def publish_waypoints(self, i):
        lane = Lane()
        lane.header = self.base_waypoints.header
        lane.waypoints = self.base_waypoints.waypoints[i:i + LOOKAHEAD_WPS]
